...
function AIInspection() {
  ...
  const analyzeVehicle = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.detail || data.error || "Vehicle analysis failed."
        );
      }
  ...
  // NUMBER PLATE RESULT
  // Backend response:
  // result.number_plate = detect_number_plate(...)
  // ---------------------------------------------------------

  const numberPlate = result?.number_plate || {};

  const plateDetections = Array.isArray(numberPlate?.detections)
  ...
  // ---------------------------------------------------------
  // LED RESULT
  // The LED detector can return either an object or a list.
  // This frontend accepts the common prediction formats.
  // ---------------------------------------------------------
  ...
  const ledPredictions = ledObjects.flatMap((item) => {
    if (Array.isArray(item?.predictions?.predictions)) {
    ...
    if (item?.class || item?.name || item?.label) {
      return [item];
    }
  ...
  const ledDetectedFromResult =
    typeof ledRaw?.detected === "boolean"
      ...
      : typeof ledRaw?.status === "string"
        ? ledRaw.status.toLowerCase().includes("detect")
        : null;
  ...
  const annotatedImageUrl = annotatedImage
    ? `${API_BASE}/inspection-results/${annotatedImage}`
    : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Detect number plates, read plate characters, and identify LED lights using AI.
        </p>
      </div>
          ...
          {loading && (
            <div className="flex min-h-[300px] items-center justify-center">
              <div className="text-center">
                ...
                <p className="mt-1 text-xs text-slate-600">
                  Detecting number plate and LED lights.
                </p>
              </div>
            </div>
          )}
                ...
                <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-violet-400">
                        {bestPlate.ocr_confidence != null
                          ? (Number(bestPlate.ocr_confidence) * 100).toFixed(1)
                          : "0.0"}%
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-4">
                    ...
                    <p className="mt-2 text-2xl font-bold tracking-[0.18em] text-cyan-300">
                      {bestPlate.plate_number || "Not Read"}
                    </p>
                  </div>
                </div>
              )}
              ...
              {!bestPlate && (
                <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-4">
                  ...
                  <p className="mt-2 text-sm text-orange-300">
                    No number plate was detected or OCR could not read it.
                  </p>
                </div>
              )}
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    >
                      {ledDetected ? "LED Light Detected" : "No LED Light Detected"}
                    </p>
                  ...
                  {ledDetected ? (
                    <Activity className="h-8 w-8 shrink-0 text-orange-400" />
                  ) : (
              ...
              {plateDetections.length > 0 && (
                <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
                  ...
                  <div className="mt-3 space-y-2">
                    {plateDetections.map((detection, index) => (
                      <div
                        key={index}
                      ...
                      >
                        <div>
                          <p className="text-xs font-medium text-white">
                            {detection.plate_number || "Number plate"}
                          </p>
                          <p className="text-[10px] text-slate-500">
                            {detection.class || "number_plate"}
                          </p>
                        </div>
